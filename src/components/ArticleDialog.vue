    ...
    <el-dialog :title="isEdit ? '编辑文章' : '新增文章'" v-model="dialogVisible" width="50%" @close="handleClose">
        <el-form :model="formData" :rules="rules" ref="formRef" label-width="120px">
            ...
            <el-form-item label="标签" prop="tags">
                <el-select v-model="formData.tagArray" placeholder="请输入标签(可选)" multiple filterable allow-create
                    style="width: 100%;">
            ...
            <el-form-item label="封面图片">
                <div class="cover-upload">
                    <el-upload class="avatar-uploader" action="#" :before-upload="beforeUpload"
                        ...
                        <img v-else :src="imgUrl" class="cover-iamge" alt="封面图片" />
                    </el-upload>
                    <!-- 移除按钮 -->
            ...
            <el-form-item label="文章内容" prop="content">
                <RichTextEditor v-model="formData.content" placeholder="请输入文章内容" :maxCharCount="50000"
                    @change="handleContentChange" @created="handleCreated" min-height="400px" />
            </el-form-item>
        </el-form>
        <div v-if="btnPreview">
            <h3>内容预览</h3>
...
const rules = reactive({
    ...
    content: [
        ...
        { max: 50000, message: '文章内容最多50000个字符', trigger: 'blur' }
    ]
})
...
const handleUploadRequest = async ({ file }) => {
    ...
    //console.log(businessId, 'UUID')

    const fileRes = await uploadFile(file, {
        businessId: businessId.value
    })
    //console.log(fileRes, '上传文件返回结果')
